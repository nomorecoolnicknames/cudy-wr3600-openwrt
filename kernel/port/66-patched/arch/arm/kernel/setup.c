...
#ifdef CONFIG_ARCH_BCMBCA
/* BCM96764 bring-up: stage marker in reset_reason[31:24], reachable before
 * paging_init through the DEBUG_LL early section mapping (virt 0xfc8xxxxx). */
static void __init bcm96764_early_mark(u8 stage)
{
	void __iomem *rr = (void __iomem *)0xfc802628UL;
	u32 v = readl_relaxed(rr);

	writel_relaxed((v & 0x00ffffffU) | ((u32)stage << 24), rr);
}
static void __init bcm96764_late_mark(u8 stage)
{
	bcm96764_early_mark(stage);	/* same virt: map_io re-mapped the periph section */
}
#else
static inline void bcm96764_early_mark(u8 s) {}
static inline void bcm96764_late_mark(u8 s) {}
#endif

void __init setup_arch(char **cmdline_p)
{
	bcm96764_early_mark(0x16);
	...
	*cmdline_p = cmd_line;
	...
	bcm96764_early_mark(0x18);
	early_fixmap_init();
	early_ioremap_init();

	parse_early_param();
	bcm96764_early_mark(0x19);
	...
	bcm96764_early_mark(0x1A);
	paging_init(mdesc);
	bcm96764_late_mark(0x1B);
	kasan_init();
